...
export default async function MovieQueryResultsComponent({
  ...
  let tmdbListResp: TMDBListResponse

  try {
    const fetchResp: Response = await fetchWithTmdbApiCreds(dataSource, {next: {revalidate}});
    tmdbListResp = await fetchResp.json() 
  ...
  return (
    <main className="flex flex-col w-full h-full min-h-0 justify-start items-stretch py-3 px-8 gap-3">
      <section className="relative flex flex-col w-full items-center justify-center shrink-0 gap-2">
        <div>
          Found <b>{tmdbListResp.total_results}</b> results...
        </div>
      ...
      <section className="flex flex-col flex-1 min-h-0 w-full">
        <MovieGrid gridItems={tmdbListResp.results} errorFetching={errorFetching}/>
      </section>

      <section className="flex w-full justify-center shrink-0">
        <Paginator
          currentPage={tmdbListResp.page}
          totalPages={tmdbListResp.total_pages}
        />
      </section>
    </main>
  );
